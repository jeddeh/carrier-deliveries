...
/**
 * Class responsible for displaying the job names and quantities (DeliveryItems)
...
public class DeliveryItemsActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		tvAddressHeader.setText(CarrierDeliveries.getDelivery(deliveryId)
				.getAddress());

		if (CarrierDeliveries.getDelivery(deliveryId).getIsDelivered() == true) {
			btnDeliveryComplete.setEnabled(false);
			btnDeliveryComplete.setBackgroundColor(Color
					.parseColor("#ff33b5e5"));
			btnDeliveryComplete.setTextColor(Color.parseColor("#323232"));
		}

		lvDeliveryItems
				.setAdapter(new DeliveryItemsAdapter(this, deliveryItems));
	}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		serviceConnector.updateDelivery(deliveryId, filePath, location);
	}
	...
	private void showEndActivityError() {
		Toast.makeText(this, "Unable to load delivery items", Toast.LENGTH_LONG)
				.show();
		finish();
	}
}
