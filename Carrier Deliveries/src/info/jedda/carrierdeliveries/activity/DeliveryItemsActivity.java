...
public class DeliveryItemsActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (CarrierDeliveries.getDelivery(deliveryId).getTimeDelivered() != null) {
	...
	public void clickDeliveryComplete(View v) {
		...
		// 250kB?? per image * 1500 images per distribution * 2 distributions per week
		// ( * ? states?? ) = estimated 750MB per week on server (?)

		// TODO : What happens if external storage is full on mobile?

		// TODO : Application is currently saving in both the '/DeliveryPhotos/' location AND the
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		// TODO : Had problems with the phone orientation changing after returning from the camera
		// intent...
		...
		case RESULT_OK:
			...
			Date timeDelivered = new Date();
			CarrierDeliveries.getDelivery(deliveryId).setTimeDelivered(timeDelivered);
	...
	public void showProgressDialog() {
		...
		// This will potentially cause multithreading problems in RestClient.java with concurrent
		// webservice requests.
